...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    bool fRescan = true;
    {
        ...
        CMichaellaoliuSecret vchSecret;
...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Michaellaoliu address or script");
        }
    }
    if (fRescan)
    {
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The michaellaoliu address for the private key\n"
            "\nResult:\n"
    ...
    return CMichaellaoliuSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to michaellaoliud)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/michaellaoliu/michaellaoliu/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Michaellaoliu %s\n", CLIENT_BUILD);
    ...
    if (!masterKeyID.IsNull())
    {
        ...
        if (pwallet->GetKey(masterKeyID, key)) {
            ...
            CMichaellaoliuExtKey b58extkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CMichaellaoliuSecret(key).ToString(), strTime);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see michaellaoliud log for details) and could "
