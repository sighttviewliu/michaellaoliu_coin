...
The RPM spec file provided here is for Michaellaoliu-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://michaellaoliu.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Michaellaoliu git repository](https://github.com/michaellaoliu/michaellaoliu) but are not part of
...
Currently only `Source10` is used, for the example `michaellaoliu.conf` file.
...
in the michaellaoliu git repository. At present only an SVG version of the Michaellaoliu
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the michaellaoliu
...
needed when 0.12.1 is released, a proper fix is already in the Michaellaoliu git
master branch.
...
idea to build Michaellaoliu Core against a static version of that library compiled
...
The `michaellaoliu.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `michaellaoliu-server` package. At this point, most RPM based
...
The `michaellaoliu-server` package should have SELinux support. How to properly do
...
Michaellaoliu-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/michaellaoliu/). He has been
packaging Michaellaoliu for Fedora at least since 2012.
...
4. I renamed the `michaellaoliu` package that contains the Qt GUI to `michaellaoliu-core` as
that appears to be how the general population refers to it, in contrast to
`michaellaoliu-xt` or `michaellaoliu-classic`. I wanted to make sure the general population
