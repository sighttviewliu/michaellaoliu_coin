#!/bin/bash
# Copyright (c) 2013 The Michaellaoliu Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Michaellaoliu database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Michaellaoliu datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Michaellaoliu datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Michaellaoliu 0.7 datadir."
    ;;
  3)
    echo "Detected Michaellaoliu pre-0.8 datadir."
    ;;
  4)
    echo "Detected Michaellaoliu 0.8 datadir."
    ;;
esac
